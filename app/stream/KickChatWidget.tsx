...
// The trailing slash is load-bearing: `next.config.js` sets `trailingSlash: true`,
// so `/api/kick-chat` costs a 308 on every single send.
const SEND_PROXY = '/api/kick-chat/';
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Only a token problem means "reconnect Kick". Slow mode, a word filter or a
        // Kick outage are all recoverable by trying again, and tearing the session
        // down for those is what made a single rejected message look like a logout.
        if (err.code === 'kick_token_expired' || err.code === 'kick_scope_missing') {
          onAuthExpired();
        }
      } else {
        setInput('');
          ...
          <div className="flex h-full flex-col items-center justify-center gap-2 px-6 text-center">
            <span
              className={`h-1.5 w-1.5 rounded-full ${
                connected ? 'bg-[#53FC18] shadow-[0_0_8px_#53FC18]' : 'animate-pulse bg-fg-faint'
              }`}
            />
            <p className="font-mono text-label uppercase text-fg-subtle">
              {connected ? 'Chat is live' : 'Connecting…'}
            </p>
            {connected && (
              // Kick's socket only pushes messages sent from now on — there is no
              // history endpoint on the public API, so an empty panel is expected
              // rather than broken, and saying so stops it reading as a failure.
